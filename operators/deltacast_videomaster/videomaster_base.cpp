...
void sleep_ms(uint32_t value) { std::this_thread::sleep_for(std::chrono::milliseconds(value)); }
...
const std::unordered_map<uint32_t, VHD_CORE_BOARDPROPERTY> id_to_firmware_loopback_prop = {
    {0, VHD_CORE_BP_FIRMWARE_LOOPBACK_0}, {1, VHD_CORE_BP_FIRMWARE_LOOPBACK_1}};

VideoMasterBase::VideoMasterBase(bool is_input, uint32_t board_index, uint32_t channel_index,
                                 bool use_rdma)
    : _is_input(is_input),
...
bool VideoMasterBase::open_stream() {
  HOLOSCAN_LOG_INFO("Opening {} stream on channel {}", _is_input ? "input" : "output",
                    _channel_index);
  ...
  success = VHD_GetBoardProperty(*board_handle(), id_to_channel_type_prop.at(_channel_index),
                                 (ULONG *)&_channel_type);
  ...
  switch (_channel_type) {
    ...
    case VHD_CHNTYPE_12GSDI:
      video_information = std::make_unique<Deltacast::Helper::SdiVideoInformation>();
      break;
    case VHD_CHNTYPE_HDMI:
    case VHD_CHNTYPE_DISPLAYPORT:
      video_information = std::make_unique<Deltacast::Helper::DvVideoInformation>();
      break;
    default:
      break;
  }
  ...
  _stream_handle = std::move(
      Deltacast::Helper::get_stream_handle(*_board_handle, id_to_stream_type.at(_channel_index),
                                           video_information->get_stream_processing_mode()));
...
bool VideoMasterBase::configure_stream() {
  bool success_b = true;
  success_b = holoscan_log_on_error(
      Deltacast::Helper::ApiSuccess{VHD_SetStreamProperty(
          *stream_handle(), VHD_CORE_SP_BUFFER_PACKING, VHD_BUFPACK_VIDEO_RGB_32)},
      "Failed to set stream type");
  if (!success_b) {
    return false;
  }

  if (!video_information->get_video_format(stream_handle())->progressive) {
    success_b = holoscan_log_on_error(Deltacast::Helper::ApiSuccess{VHD_SetStreamProperty(
                                          *stream_handle(), VHD_CORE_SP_FIELD_MERGE, TRUE)},
  ...
  if (_is_input) {
    auto success_opt = video_information->set_stream_properties_values(
        *_stream_handle, video_information->get_stream_properties_values(*_stream_handle));
  ...
  const auto &id_to_stream_type = _is_input ? id_to_rx_stream_type : id_to_tx_stream_type;
  video_format = video_information->get_video_format(*_stream_handle).value();
...
bool VideoMasterBase::init_buffers() {
  ...
  if (_use_rdma || _is_input) {
    for (auto &slot : gpu_buffers) slot.resize(video_information->get_nb_buffer_types());
  }

  if (!_use_rdma) {
    for (auto &slot : system_buffers) slot.resize(video_information->get_nb_buffer_types());
  }
  ...
  buffer_sizes.resize(video_information->get_nb_buffer_types());
  HOLOSCAN_LOG_DEBUG("Buffer types count: {}, Slots count: {}",
                     video_information->get_nb_buffer_types(), gpu_buffers.size());

  for (int buffer_type_index = 0; buffer_type_index < video_information->get_nb_buffer_types();
       buffer_type_index++) {
    VHD_GetApplicationBuffersSize(*stream_handle(), buffer_type_index,
                                  &buffer_sizes[buffer_type_index]);
    HOLOSCAN_LOG_DEBUG("Buffer type {}: size = {} bytes", buffer_type_index,
                       buffer_sizes[buffer_type_index]);
    if (!buffer_sizes[buffer_type_index]) continue;

    HOLOSCAN_LOG_DEBUG("Is IGPU: {}", _is_igpu);
    for (int slot_index = 0; slot_index < gpu_buffers.size(); slot_index++) {
      if ((buffer_type_index == video_information->get_buffer_type() && _use_rdma) || _is_input) {
        cudaError_t cuda_error;
        if (_is_igpu) {
          HOLOSCAN_LOG_DEBUG(
              "Trying to allocate Slot {}, Buffer type {}: "
              "CUDA host alloc (iGPU) - {} bytes",
              slot_index, buffer_type_index, buffer_sizes[buffer_type_index]);
          cuda_error = cudaHostAlloc(&gpu_buffers[slot_index][buffer_type_index],
                                     buffer_sizes[buffer_type_index], cudaHostAllocDefault);
          if (cuda_error != cudaSuccess) {
          ...
          HOLOSCAN_LOG_DEBUG(
              ...
              slot_index, buffer_type_index, buffer_sizes[buffer_type_index]);
          cuda_error = cudaMalloc(&gpu_buffers[slot_index][buffer_type_index],
                                  buffer_sizes[buffer_type_index]);
          if (cuda_error != cudaSuccess) {
      ...
      if (buffer_type_index == video_information->get_buffer_type() && !_use_rdma) {
        HOLOSCAN_LOG_DEBUG(
            "Trying to allocate Slot {}, Buffer type {}: "
            "posix_memalign alloc for VideoMaster DMA - {} bytes",
            slot_index, buffer_type_index, buffer_sizes[buffer_type_index]);
        if (posix_memalign((void **)&system_buffers[slot_index][buffer_type_index], 4096,
                           buffer_sizes[buffer_type_index])) {
  ...
  for (int slot_index = 0; slot_index < slot_handles.size(); slot_index++) {
    HOLOSCAN_LOG_DEBUG("Creating slot {}/{}", slot_index + 1, slot_handles.size());
    ...
    for (int buffer_type_index = 0; buffer_type_index < video_information->get_nb_buffer_types();
      ...
      if (buffer_type_index == video_information->get_buffer_type() && !_use_rdma) {
        desc.pBuffer =
            (buffer_sizes[buffer_type_index] ? system_buffers[slot_index][buffer_type_index]
                                             : nullptr);
      } else {
        desc.pBuffer = (buffer_sizes[buffer_type_index] ? gpu_buffers[slot_index][buffer_type_index]
                                                        : nullptr);
      }
      desc.RDMAEnabled = (buffer_type_index == video_information->get_buffer_type() && _use_rdma);
      HOLOSCAN_LOG_DEBUG("  Buffer type {}: ptr={}, RDMA={}", buffer_type_index,
                         (void *)desc.pBuffer, desc.RDMAEnabled);

      raw_buffer_pointer.push_back(desc);
    }
    HOLOSCAN_LOG_DEBUG("Creating slot with {} buffer types", raw_buffer_pointer.size());
    success =
        VHD_CreateSlotEx(*stream_handle(), raw_buffer_pointer.data(), &slot_handles[slot_index]);
    if (!success) {
    ...
    if (_is_input) {
      success = VHD_QueueInSlot(slot_handles[slot_index]);
      if (!success) {
...
void VideoMasterBase::free_buffers() {
  ...
  for (int slot_index = 0; slot_index < gpu_buffers.size(); slot_index++) {
    for (int buffer_type_index = 0; buffer_type_index < gpu_buffers[slot_index].size();
         buffer_type_index++) {
      if (gpu_buffers[slot_index][buffer_type_index] != nullptr) {
        if (_is_igpu) {
          cudaError_t cuda_error = cudaFreeHost(gpu_buffers[slot_index][buffer_type_index]);
          if (cuda_error != cudaSuccess) {
            HOLOSCAN_LOG_ERROR("CUDA host free failed for slot {}, buffer type {}: {}", slot_index,
                               buffer_type_index, cudaGetErrorString(cuda_error));
          } else {
            HOLOSCAN_LOG_DEBUG("Freed CUDA host buffer for slot {}, buffer type {}", slot_index,
                               buffer_type_index);
          }
        } else {
          cudaError_t cuda_error = cudaFree(gpu_buffers[slot_index][buffer_type_index]);
          if (cuda_error != cudaSuccess) {
            ...
            HOLOSCAN_LOG_DEBUG("Freed CUDA device buffer for slot {}, buffer type {}", slot_index,
                               buffer_type_index);
          }
        }
        gpu_buffers[slot_index][buffer_type_index] = nullptr;
      }
    }
  }

  // Free system buffers (only allocated in non-RDMA mode)
  for (int slot_index = 0; slot_index < system_buffers.size(); slot_index++) {
    for (int buffer_type_index = 0; buffer_type_index < system_buffers[slot_index].size();
         buffer_type_index++) {
      if (system_buffers[slot_index][buffer_type_index] != nullptr) {
        free(system_buffers[slot_index][buffer_type_index]);
        HOLOSCAN_LOG_DEBUG("Freed system buffer for slot {}, buffer type {}", slot_index,
                           buffer_type_index);
        system_buffers[slot_index][buffer_type_index] = nullptr;
      }
    }
  }
}
...
bool VideoMasterBase::holoscan_log_on_error(Deltacast::Helper::ApiSuccess result,
  ...
  if (!result_b) {
    ...
    std::string error_message =
        message + " - Error: " + Deltacast::Helper::enum_to_string(error_code);
...
bool VideoMasterBase::set_loopback_state(bool state) {
  ...
  success_b = success_b &
              holoscan_log_on_error(
                  Deltacast::Helper::ApiSuccess{VHD_GetBoardCapability(
                      *board_handle(), VHD_CORE_BOARD_CAP_PASSIVE_LOOPBACK, &has_passive_loopback)},
                  "Failed to retrieve passive loopback capability");

  success_b = success_b &
              holoscan_log_on_error(
                  Deltacast::Helper::ApiSuccess{VHD_GetBoardCapability(
                      *board_handle(), VHD_CORE_BOARD_CAP_ACTIVE_LOOPBACK, &has_active_loopback)},
                  "Failed to retrieve active loopback capability");

  success_b =
      success_b & holoscan_log_on_error(Deltacast::Helper::ApiSuccess{VHD_GetBoardCapability(
                                            *board_handle(), VHD_CORE_BOARD_CAP_FIRMWARE_LOOPBACK,
  ...
  if (has_firmware_loopback &&
    ...
    success_b = holoscan_log_on_error(
        Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
            *board_handle(), id_to_firmware_loopback_prop.at(_channel_index), state)},
    ...
    success_b = holoscan_log_on_error(
        Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
            *board_handle(), id_to_active_loopback_prop.at(_channel_index), state)},
        "Failed to set active loopback state");

    return success_b;
  } else if (has_passive_loopback && id_to_passive_loopback_prop.find(_channel_index) !=
                                         id_to_passive_loopback_prop.end()) {
    success_b = holoscan_log_on_error(
        Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
            *board_handle(), id_to_passive_loopback_prop.at(_channel_index), state)},
