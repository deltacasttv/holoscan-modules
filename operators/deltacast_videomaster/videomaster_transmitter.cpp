...
void VideoMasterTransmitterOp::setup(OperatorSpec& spec) {
  ...
  spec.param(_progressive, "progressive", "Progressive",
             "Progressiveness of the video frames to send.", true);
  ...
  spec.param(_overlay, "enable_overlay", "EnableOverlay",
...
void VideoMasterTransmitterOp::initialize() {
  ...
  _video_master_base =
      std::make_unique<VideoMasterBase>(false, _board_index, _channel_index, _use_rdma);
}

void VideoMasterTransmitterOp::start() {
  HOLOSCAN_LOG_INFO("Starting VideoMaster Transmitter on board {} channel {}", _board_index,
                    _channel_index);
  if (!_video_master_base->configure_board()) throw std::runtime_error("Failed to configure board");
  if (!_video_master_base->open_stream()) throw std::runtime_error("Failed to open stream");

  if (!_overlay) {
    _video_master_base->video_format =
        Deltacast::Helper::VideoFormat{_width, _height, _progressive, _framerate};
    _video_master_base->video_information->set_video_format(_video_master_base->stream_handle(),
                                                            _video_master_base->video_format);

    auto opt_sync_source_property =
        _video_master_base->video_information->get_sync_source_properties();
    if (opt_sync_source_property)
      VHD_SetBoardProperty(*_video_master_base->board_handle(), *opt_sync_source_property,
                           VHD_GENLOCK_LOCAL);
    ...
    if (!_video_master_base->start_stream()) throw std::runtime_error("Failed to start stream");
...
void VideoMasterTransmitterOp::compute(InputContext& op_input, OutputContext& op_output,
                                       ExecutionContext& context) {
  HOLOSCAN_LOG_DEBUG("VideoMasterTransmitterOp::compute() - Starting compute cycle {}",
                     _slot_count);
  ...
  if (_overlay) {
    if (!_video_master_base->signal_present()) {
      if (!_has_lost_signal) HOLOSCAN_LOG_INFO("No signal detected, waiting for input...");

      _has_lost_signal = true;
      return;
    } else if (!(_video_master_base->video_format !=
                 Deltacast::Helper::VideoFormat{})) {  // stream not started yet
      HOLOSCAN_LOG_INFO("Configuring overlay mode: {}x{} {} @ {} Hz", _width, _height,
  ...
  HANDLE slot_handle;
  HOLOSCAN_LOG_DEBUG("Getting slot handle - slot_count: {}, available slots: {}", _slot_count,
                     _video_master_base->slot_handles.size());
  if (_slot_count >= _video_master_base->slot_handles.size()) {
    ...
    if (!_video_master_base->holoscan_log_on_error(
            Deltacast::Helper::ApiSuccess{VHD_WaitSlotSent(
                *_video_master_base->stream_handle(), &slot_handle, VideoMasterBase::SLOT_TIMEOUT)},
    ...
    HOLOSCAN_LOG_DEBUG("Using available slot handle - index: {}, slot_handle: {}",
                       _slot_count % VideoMasterBase::NB_SLOTS, static_cast<void*>(slot_handle));
  }

  BYTE* buffer = nullptr;
  ULONG buffer_size = 0;

  HOLOSCAN_LOG_DEBUG("Attempting to get video buffer - slot_handle: {}",
                     static_cast<void*>(slot_handle));
  ...
  HOLOSCAN_LOG_DEBUG(
      "Copying data to buffer - tensor size: {}, buffer size: {}, "
      "copy type: {}",
  ...
  cudaError_t cuda_result =
      cudaMemcpy(buffer, tensor->data(), tensor->nbytes(),
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_QueueOutSlot(slot_handle)},
...
bool VideoMasterTransmitterOp::configure_board_for_overlay() {
  ...
  if (!_video_master_base->video_information->configure_sync(_video_master_base->board_handle(),
                                                             _channel_index)) {
    return false;
  }
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
              *_video_master_base->board_handle(), keyer_props.at(VHD_KEYER_BP_INPUT_A),
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
              *_video_master_base->board_handle(), keyer_props.at(VHD_KEYER_BP_INPUT_B),
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
              *_video_master_base->board_handle(), keyer_props.at(VHD_KEYER_BP_INPUT_K),
              id_to_tx_keyer_input.at(_channel_index))},
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{
              VHD_SetBoardProperty(*_video_master_base->board_handle(),
                                   id_to_keyer_video_output.at(_channel_index), VHD_KOUTPUT_KEYER)},
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
              *_video_master_base->board_handle(), keyer_props.at(VHD_KEYER_BP_ALPHACLIP_MIN), 0)},
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{
              VHD_SetBoardProperty(*_video_master_base->board_handle(),
                                   keyer_props.at(VHD_KEYER_BP_ALPHACLIP_MAX), 1020)},
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{
              VHD_SetBoardProperty(*_video_master_base->board_handle(),
                                   keyer_props.at(VHD_KEYER_BP_ALPHABLEND_FACTOR), 1023)},
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetBoardProperty(
              *_video_master_base->board_handle(), keyer_props.at(VHD_KEYER_BP_ENABLE), TRUE)},
...
bool VideoMasterTransmitterOp::configure_stream_for_overlay() {
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_SetStreamProperty(*_video_master_base->stream_handle(),
  ...
  if (opt_sync_tx_property) {
    if (!_video_master_base->holoscan_log_on_error(
            Deltacast::Helper::ApiSuccess{VHD_SetStreamProperty(
                *_video_master_base->stream_handle(), *opt_sync_tx_property, TRUE)},
