...
void VideoMasterSourceOp::initialize() {
  ...
  _video_master_base =
      std::make_unique<VideoMasterBase>(true, _board_index, _channel_index, _use_rdma);
}

void VideoMasterSourceOp::start() {
  HOLOSCAN_LOG_INFO("Starting VideoMaster Source on board {} channel {}", _board_index,
                    _channel_index);
...
void VideoMasterSourceOp::compute(InputContext& op_input, OutputContext& op_output,
  ...
  auto detected_video_format =
      _video_master_base->video_information->get_video_format(_video_master_base->stream_handle());
  ...
  ULONG api_result = VHD_WaitSlotFilled(*_video_master_base->stream_handle(), &slot_handle,
  ...
  if (!_video_master_base->holoscan_log_on_error(
          Deltacast::Helper::ApiSuccess{VHD_GetSlotBuffer(
              slot_handle, _video_master_base->video_information->get_buffer_type(), &buffer,
              &buffer_size)},
...
void VideoMasterSourceOp::transmit_buffer_data(void* buffer, uint32_t buffer_size,
  ...
  if (!_use_rdma) {
    ...
    void* gpu_buffer =
        _video_master_base->gpu_buffers[_slot_count % VideoMasterBase::NB_SLOTS]
  ...
  auto storage_type = _video_master_base->is_igpu() ? nvidia::gxf::MemoryStorageType::kHost
